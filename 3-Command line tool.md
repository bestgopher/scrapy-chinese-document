...
Scrapy通过scrapy命令行工具控制，这里称为'Scrapy tool'以区分'commands'或者'Scrapy commands'等子命令。
...
Scrapy将会在特定位置中的ini样式文件scrapy.cfg中寻找配置参数：
...
scrapy.cfg文件所在的目录被称为项目的根目录。scrapy.cfg文件包含python模块的名字，定义了项目的设置。如下所示：

	[settings]
	default = myproject.settings

## 使用scrapy工具(Using scrapy tool) ##
...
通常你用scray工具做的第一件事就是创建项目：

    scrapy startproject myproject [project_dir]

这将会创建一个项目在 <font color=red>project\_dir</font> 目录下。如果 <font color=red> project\_dir</font> 没有指定， <font color=red>project_dir</font> 将会和
...
这里你准备好使用 scrapy 命令去管理和控制你的项目了。

## 控制项目(Controlling projects) ##

你使用你项目内部的scrapy工具来管理和控制它们。
...
还要记住的是，有些命令在项目中运行可能会有细微差别的行为。比如，<font color=red>fetch</font> 命令提取的url与某些爬虫相关联的话，则会使用这个爬虫的动作(比如UA会被覆盖)。这是有意为之的，<font color=red>fetch</font> 命令就是用来测试检查爬虫如何下载页面的。
...
在 <font color=red>project\_dir</font> 目录下创建名为 <font color=red>project\_name</font> 的项目。如果 <font color=red> project\_dir</font> 没有指定， <font color=red>project_dir</font> 将会和
 <font color=red>project\_name</font> 一样。
...
支持的选项：
 ...
 - <font color=red>--no-redirect</font>：不允许网页重定向，只对开始shell的url有用。当进入shell中使用 <font color=red>fetch</font> 提取网页时，依然允许网页重定向。
